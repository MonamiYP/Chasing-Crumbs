...
enum STATES { IDLE, MOVE, CHAT }
var current_key : String = "First"
...
func update_key() -> void:
	var event : Dictionary = GameManager.get_event(self.npc_resource.npc_name, GameManager.current_scene)
	if current_key != 'Final':
		current_key = event[current_key]
